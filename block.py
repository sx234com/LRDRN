...
class LFFEM_C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion  假设c1=256 c2=512
        ...
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(LFFEM_Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class LFFEM_Bottleneck(nn.Module):
...
#Fine-grained channel attention mechanism (FGCA)
class FGCAattention(nn.Module):
    def __init__(self,channel,b=1, gamma=2):
        super(FGCAattention, self).__init__()
    ...
    def forward(self, input):
        ...
        # x2_x1_trans_mul_sum = torch.sum(x2_x1_trans_mul, dim=1)
        # x2_x1_trans_mul_sum_squ1 = x2_x1_trans_mul_sum.unsqueeze(-1)
        # x2_x1_trans_mul_sum_squ2 = x2_x1_trans_mul_sum_squ1.unsqueeze(-1)
        # out2 = x2_x1_trans_mul_sum_squ2
        # # 激活一下
...
# lightweight frequency-domain feature extraction module (LFFEM)
class LFFEM(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=4, dw_size=3, stride=1, relu=True):
        super(LFFEM, self).__init__()
        self.oup = oup  #in 48 out 32 ratio
        selected_num=int(inp // ratio) #selected_num=12
        ...
        if(selected_num>0):
            # 开启选择的情况下
            self.CSS=CSSModule(kernel_size=3,a=ratio)

            # FDES
            self.ffc = FFC(in_channels=selected_num, out_channels=selected_num, kernel_size=kernel_size, ratio_gin=0, ratio_gout=0.5, stride=stride)
            ...
            # FDES
            self.ffc = FFC(in_channels=selected_num, out_channels=selected_num, kernel_size=kernel_size,
                                  ratio_gin=0, ratio_gout=0.5, stride=stride)


            # FRS
            self.feature_reuse = nn.Sequential(
                nn.Conv2d(selected_num, remain_num, dw_size, 1, dw_size // 2, groups=selected_num, bias=False),  ##使用一维卷积作为线性计算
                nn.BatchNorm2d(remain_num),
                nn.ReLU(inplace=True) if relu else nn.Sequential(),
            )
